...
module.exports = function plugin(command, targets, opts) {
    ...
    return Q().then(function () {
        ...
        var config_json = config.read(projectRoot);
         ...
         var platformList = [];
        if(cordova_util.isWeexPluginRoot(projectRoot)){
            if(cordova_util.isIOSProject(projectRoot)){
       ...
       // WEEX_HOOK
        ...
        switch(command) {
            case 'add':
                ...
                //创建一个新的
                if(!xml&&cordova_util.isWeexPluginRoot(projectRoot)){
                    var xmlPath = path.resolve(__dirname,"../../../generator/templates/config.xml")
                    shell.cp('-R', xmlPath, projectRoot);
                }
                    ...
                    return opts.plugins.reduce(function(soFar, target) {
                        return soFar.then(function() {
                            ...
                            .then(function(resolvedTarget) {
                                ...
                                return plugman.raw.fetch(target, pluginPath, fetchOptions, projectRoot);
                            })
