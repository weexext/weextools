...
// Returns a promise.
module.exports = function installPlugin(platform, project_dir, id, plugins_dir,source_dir, options) {
    ...
    .then(function(plugin_dir) {
        return runInstall(current_stack, platform, project_dir, plugin_dir, plugins_dir, source_dir,options);
    });
};
...
function runInstall(actions, platform, project_dir, plugin_dir, plugins_dir,source_dir, options) {
    project_dir = cordovaUtil.convertToRealPathSafe(project_dir);
    ...
    return Q().then(function() {
        ...
        function(){
            ...
            if ( !fs.existsSync(install_plugin_dir) ) {
                ...
                //copyPlugin(path.join(plugin_dir), source_dir, options.link, pluginInfoProvider);
            }
